...
public class ClientOperation {
    ...
    public List<Food> getFoodsByRestaurantId(int idRest) {
        ...
        try {
            String sql = "SELECT * FROM test.food WHERE idRest = ?";
    ...
    public void registerRestaurant(Restaurant r) {
        ...
        try {
            ...
            String sql2 = "INSERT INTO food (idRest, food, price) VALUES (?, ?, ?)";
            PreparedStatement stmt2 = connection.prepareStatement(sql2);

            for (Food f : r.getFoods()) {
                stmt2.setInt(1, r.getId());
                stmt2.setString(2, f.getFood());
    ...
    public void updateRestaurant(int clientId, String name, String address, String email, String[] foods, double[] prices) {
        try {
            ...
            // Loop through each food item and update
            for (int i = 0; i < foods.length; i++) {
                updateFood(foodIds[i], foods[i], prices[i]);
            }
            ...
            for (Restaurant restaurant : rest) {
                if (restaurant.getId() == clientId) {
                    ...
                    for (int i = 0; i < foods.length; i++) {
                        updatedFoodList.get(i).setFood(foods[i]);
    ...
    public void updateFood(int foodId, String food, double price) {
        try {
            String query = "UPDATE food SET food = ?, price = ? WHERE idFood = ?";
            PreparedStatement pstmt = connection.prepareStatement(query);
